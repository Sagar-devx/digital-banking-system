...
class TransactionServiceImplTest {
    ...
    void transfer_DeductFailed() {
        ...
        RuntimeException exception = assertThrows(RuntimeException.class, () -> transactionService.transfer(request));
    ...
    @Test
    @DisplayName("Get transaction throws when not found")
    void getTransaction_NotFound() {
        when(transactionRepository.findById("tx-999")).thenReturn(Optional.empty());

        RuntimeException ex = assertThrows(RuntimeException.class, () -> transactionService.getTransaction("tx-999"));
    ...
    void getTransactionHistory_Success() {
        ...
        when(transactionRepository.findBySenderAccountNumberOrderByCreatedAtDesc(account)).thenReturn(List.of(tx1, tx2));
    ...
    @Test
    @DisplayName("Verify OTP throws when transaction not found")
    void verifyOTP_NotFound() {
        when(transactionRepository.findById("tx-999")).thenReturn(Optional.empty());

        RuntimeException ex = assertThrows(RuntimeException.class, () -> transactionService.verifyOTP("tx-999", "123456"));
        assertEquals("Transaction not found", ex.getMessage());
    }
    ...
    @Test
    @DisplayName("Process clean result throws when transaction not found")
    void processCleanResult_NotFound() {
        when(transactionRepository.findById("tx-999")).thenReturn(Optional.empty());

        RuntimeException ex = assertThrows(RuntimeException.class, () -> transactionService.processCleanResult("tx-999"));
        assertEquals("Transaction not found", ex.getMessage());
    }
}