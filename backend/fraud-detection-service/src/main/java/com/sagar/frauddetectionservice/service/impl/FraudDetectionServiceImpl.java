...
import com.sagar.frauddetectionservice.exception.InvalidTransactionPayloadException;
import com.sagar.frauddetectionservice.model.FraudCheckResult;
import com.sagar.frauddetectionservice.service.FraudDetectionService;
...
public class FraudDetectionServiceImpl implements FraudDetectionService {
    ...
    private static final String FRAUD_CHECK_CLEAR_RESULT_TOPIC = "fraud.check.clean";
    ...
    public void checkTransaction(Map<String, Object> payload) {
        if (payload == null || payload.get("transactionId") == null || payload.get("senderAccountNumber") == null || payload.get("amount") == null) {
            throw new InvalidTransactionPayloadException("Invalid transaction payload: transactionId, senderAccountNumber, and amount are required");
        }

        String transactionId = String.valueOf(payload.get("transactionId"));
        String accountNumber = String.valueOf(payload.get("senderAccountNumber"));
        BigDecimal amount = new BigDecimal(payload.get("amount").toString());
        String description = payload.get("description") != null ? payload.get("description").toString() : "";

        BigDecimal senderBalance = accountServiceClient.getBalance(accountNumber);

        log.info("Checking transaction {} for account {} with amount {} and description {}. Sender balance is {}", transactionId, accountNumber, amount, description, senderBalance);

        FraudCheckResult result = performFraudCheck(accountNumber, amount, senderBalance);

        if (result.isFraud()) {

            log.info("Suspicious activity detected for account {} - reason: {} - requesting OTP verification", accountNumber, result.getReason());
        ...
        else
        {
            ...
            log.info("Transaction {} for account {} is clean. No further action required.", transactionId, accountNumber);
            Map<String, Object> transactionCleanEvent = new HashMap<>();
            transactionCleanEvent.put("transactionId", transactionId);
            transactionCleanEvent.put("isFraud",false);
            transactionCleanEvent.put("reason",null);
            kafkaTemplate.send(FRAUD_CHECK_CLEAR_RESULT_TOPIC, transactionId, transactionCleanEvent);
        }
    }
    ...
    private FraudCheckResult performFraudCheck(
        ...
        {
            return new FraudCheckResult(true, "Too many transactions in a short period" + "- Velocity limit exceeded");
        }
    ...
    private boolean isAmountSuspicious(String accountNumber, BigDecimal amount) {
        ...
        log.info("Account check - amount: {} threshold: {} suspicious: {}", amount, threshold, amount.compareTo(threshold) > 0);
    ...
    private boolean isVelocityExceeded(String accountNumber) {
        ...
        log.info("velocity checks - account: {} count: {}/{}", accountNumber, count,maxTransactionPerMinute);
